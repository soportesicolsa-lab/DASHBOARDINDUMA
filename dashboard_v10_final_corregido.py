...
@st.cache_data
def load_data():
    excel_file = '/home/ubuntu/upload/Tactico_9box(1).xlsx'
        ...
        if es_jefe_empleado:
            ...
            if not equipo.empty:
                ...
                equipo_con_evaluacion = equipo[
                    equipo.get('Potencial', pd.Series()).notna() & 
                    equipo.get('Desempeño', pd.Series()).notna()
                ]
...
empleados_con_evaluacion = empleados_filtrados[
    empleados_filtrados.get('Potencial', pd.Series()).notna() & 
    empleados_filtrados.get('Desempeño', pd.Series()).notna()
]
